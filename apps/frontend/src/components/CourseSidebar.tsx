...
import { CheckCircle, Circle, Lock, PlayCircle, FileText, Trophy, HelpCircle } from "lucide-react";
...
export function CourseSidebar({ course, currentLessonId, onSelectLesson, completedLessonIds = [], isEnrolled = false }: CourseSidebarProps) {
    return (
        <div className="h-full border-r bg-card overflow-y-auto">
            ...
            <Accordion type="multiple" defaultValue={course.modules.map((m: any) => m.id)} className="w-full">
                {course.modules.map((module: any, index: number) => {
                    const isModuleUnlocked = isEnrolled || index === 0;
                            ...
                            <AccordionContent className="pt-0 pb-0">
                                <div className="flex flex-col">
                                    {module.lessons.map((lesson: any) => {
                                        ...
                                        // Lock logic: Unlocked if Enrolled OR Module 1.
                                        // Specific lesson limit (Top 5) can be handled here if needed, but Module 1 is safer.
                                    ...
                                    {module.tests?.map((test: any) => {
                                        const isActive = currentLessonId === test.id;
                                        const isLocked = !isModuleUnlocked;

                                        return (
                                            <button
                                                key={test.id}
                                                onClick={() => onSelectLesson(test)}
                                                className={cn(
                                                    "flex items-start gap-3 px-4 py-3 text-sm transition-colors border-l-2",
                                                    isActive
                                                        ? "bg-amber-50/50 border-amber-500 text-amber-700"
                                                        : "border-transparent hover:bg-muted/50 text-foreground/80",
                                                    isLocked && "opacity-75"
                                                )}
                                            >
                                                <div className="mt-0.5 shrink-0">
                                                    {isLocked ? (
                                                        ...
                                                        <Trophy className={cn("h-4 w-4", isActive ? "text-amber-500" : "text-muted-foreground/40")} />
                                                    )}
                                                </div>
                                                <div className="flex-1 text-left">
                                                    ...
                                                    <div className="flex items-center gap-2 mt-1.5 text-[10px] text-muted-foreground">
                                                        <span className="flex items-center gap-1"><HelpCircle className="h-3 w-3" /> Quiz</span>
